...
def test_bigram_model():
    ...
    # Process test data
    df_test = conllu_to_df('./data/corpus-test.conllu')
    ...
    for sent_id, predicted_words in result.items():
        ...
        print(df_test.loc[df_test['sent_id'] == sent_id, 'form'].tolist())
        break
    
    df_eval = conllu_to_df('./data/corpus-dev.conllu')
    ...
    acuracy, precision, recall, f1_score = bigram.evaluate(df_eval)
    print(f"Acurácia: {acuracy}, Precisão: {precision}, Cobertura: {recall}, Medida F1: {f1_score}")
